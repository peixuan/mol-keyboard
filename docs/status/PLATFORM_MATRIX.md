...
| Linux x86_64 | runtime-verified (M5 service, WSL/null audio) | 2026-09-03 | WSL; Clang 21.1.8 | Static 76/76 and shared 74/74; exact 47-symbol allow-list; ABI binary/source compatibility 100%; independent daemon/CLI over mode-0600 Unix socket; evdev and native audio hardware not exposed |
| Linux aarch64 | build-verified | 2026-09-03 | Ubuntu resolute cross sysroot; GNU 15.2.0; WSL x86_64 cross host | Checked-in preset produced AArch64 ELF `mol_core`, daemon, CLI, playback, sequence, render, patch, and analyzer outputs; a native Ubuntu ARM64 CI runner is configured, but runtime/audio/input remain unverified until that job or a device run completes |
