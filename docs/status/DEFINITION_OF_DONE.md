...
The latest fully green executable source candidate is commit `95c6cee`; subsequent
...
| 8 | Windows, Linux, and macOS headless service and CLI | verified | Independent daemon/CLI lifecycles and packages pass on Windows and Linux; native macOS arm64 and x86_64 jobs pass the real temporary LaunchAgent null-audio lifecycle and extracted-package service audit. See [`M5_DESKTOP_EVIDENCE.md`](../service/M5_DESKTOP_EVIDENCE.md). |
| 9 | Offline AudioWorklet + Wasm Web/PWA | verified | Production bundle, offline install/reload, Wasm DSP, AudioWorklet, MessagePort/SAB, keyboard/touch, lifecycle, and Chrome/Edge/Firefox/current-Safari automation pass in [`M6_WEB_EVIDENCE.md`](../web/M6_WEB_EVIDENCE.md). |
...
| 11 | iOS AudioUnit/AVAudioSession and legal background audio | implementation verified; external acceptance open | Warnings-as-errors Simulator/device builds, packaged-resource audits, the launched iPhone Simulator UI/reply bridge, portable production lifecycle/key state machines, and sanitizers pass. Physical audio, background/lock-screen lifecycle, routes, input, signing, endurance, and latency remain open in [`M7_IOS_EVIDENCE.md`](../mobile/M7_IOS_EVIDENCE.md). |
...
| 18 | Coverage, sanitizers, static analysis, and long tests | implementation verified; external acceptance open | Core coverage is 93.94% with every critical module at or above 95%; 46 production translation units, the complete sanitizer/fuzzer suite, 64 ThreadSanitizer tests, and optimized 30-minute/recovery tests pass. Physical mobile and ESP32 endurance is still required by platform acceptance; see [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md). |
...
| 20 | Reproducible documented clean-checkout builds | implementation verified; external acceptance open | Exact clean Windows, Linux, macOS arm64/x86_64, Wasm, Web, Android, iOS, and ESP-IDF builds pass with recorded package hashes or bundle audits. A formal HarmonyOS clean build still requires its authenticated official environment; see [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md). |
| 21 | Every support claim backed by real evidence | open | Windows/Linux/macOS x64 and ARM64, supported Web browsers, iPhone and Android simulators, AArch64 QEMU, OpenHarmony compatibility, and Espressif QEMU are qualified at their actual levels. Formal HarmonyOS, physical mobile/ESP32/peripherals/routes, and required latency evidence are absent; see [`PLATFORM_MATRIX.md`](PLATFORM_MATRIX.md). |
...
| 24 | Publish truthful `v1.0.0` with matching notes | open | [`v1.0.0.md`](../releases/v1.0.0.md) remains an explicit draft and the repository has no `v1.0.0` tag. Publishing is forbidden until every externally open item above closes. |
...
remaining evidence requires an authenticated current HarmonyOS toolchain/session,
physical macOS/Android/iOS/Harmony/ESP32 targets, routes, and peripherals,
