...
not yet a realtime desktop host, ESP32 I2S audio task, service, complete PWA, or
end-user application. The ESP-IDF firmware is compile-verified and includes a
startup render self-test, but has not run on a physical board. PCM24 and float
WAV output are not implemented yet.
