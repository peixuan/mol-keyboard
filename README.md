...
| --- | --- | --- |
| Windows | wxWidgets/WebView2 instrument, native debugger, daemon, CLI, WASAPI, Raw Input, WinMM MIDI | x64 GUI/package/Startup lifecycle and native ARM64 headless suites pass; physical MIDI/Bluetooth pending / x64 GUI、安装包、启动服务生命周期及原生 ARM64 无界面套件通过；物理 MIDI/蓝牙待验 |
| Linux | wxWidgets/WebKitGTK instrument, native debugger, daemon, CLI, native audio/evdev/raw-MIDI host | x86_64 GUI/package/systemd, native AArch64 headless, and AArch64 QEMU suites pass; physical display/audio/input pending / x86_64 GUI、安装包、systemd、原生 AArch64 无界面及 AArch64 QEMU 套件通过；物理显示、音频与输入待验 |
| macOS | wxWidgets/WKWebView instrument, native debugger, daemon, CoreAudio, IOHIDManager, CoreMIDI | hosted arm64/x86_64 GUI, LaunchAgent, package, and independent headless lifecycles pass; physical I/O/routes pending / 托管 arm64/x86_64 GUI、LaunchAgent、安装包与独立无界面生命周期通过；物理输入输出与路由待验 |
| Web/PWA | Wasm AudioWorklet, offline shell | Chrome, Edge, Firefox, and actual current Safari automation passed at their documented capability levels / Chrome、Edge、Firefox 及真实当前 Safari 自动化均按文档能力边界通过 |
...
| iOS | AudioUnit, AVAudioSession, offline WKWebView | warnings-as-errors Simulator/device builds and launched Simulator UI/bridge pass; physical device pending / 警告即错误的模拟器/设备构建及实际启动的模拟器 UI/桥接通过；真机待验 |
